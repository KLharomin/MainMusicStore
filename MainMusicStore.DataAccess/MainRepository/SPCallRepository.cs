...
namespace MainMusicStore.DataAccess.MainRepository
{
    public class SPCallRepository : ISPCallRepository
    {
        ...
        public SPCallRepository(ApplicationDbContext db)
        {
            ...
            //Bu ef den biraz farklı çalışıyor burada connectionstring vermen gerekiyor.
        }

        public void Dispose()
        {
            _db.Dispose();
            //işin bittikten sonra dpyi at demektir bu kadar
        }
        ...
        public T OneRecord<T>(string procedureName, DynamicParameters parameters = null)
        {
            ...
            {
                sqlCon.Open();
                var value =  sqlCon.Query(procedureName, parameters, commandType: System.Data.CommandType.StoredProcedure);
                return (T)Convert.ChangeType(value.FirstOrDefault(), typeof(T));
                //change type için bir datayı veriyorsun birde dönüşeceği tipi veriyorsun aslında.
            }
        }
        ...
        public T Single<T>(string procedureName, DynamicParameters parameters = null)
        {
            using (SqlConnection sqlCon = new SqlConnection(connectionString))
            {
                ...
                //sorguyu ve dönüşeceği tipi kendi içinde alıyor aslında burada.
